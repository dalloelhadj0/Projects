...
int main() {
    ...
    cout << "TEST_CASE_INSTRUCTOR: "<<endl;
    cout << "PART_1 EXECUTION "<<endl;
    ifstream infile1("My_Driver_Test_Part_1.txt");
    if (!infile1) {
    ...
    // part 2
    cout << "PART_2 EXECUTION ";
    ifstream infile2("My_Driver_Test_Part_2.txt");
    if (!infile2) {
        cout << "File could not be Opened." << endl;
        return 1;
    }
    ...
    for (;;) {
        ...
        if (infile2.eof())
            break;
        G.DisplayDFS();
        G.DFS();    // find and display depth-first ordering to cout
    }
